...
const createAlertTable = (id, data) => {
  ...
  row1_data2.innerHTML = `${date.getFullYear()}-${(date.getMonth() < 10) ? "0" : ""}${date.getMonth() + 1}-${(date.getDate() < 10) ? "0" : ""}${date.getDate()} ${date.getHours()}:${date.getMinutes()}:${date.getSeconds()} ${Intl.DateTimeFormat().resolvedOptions().timeZone}`;
